...
/**
 * Normalize Markdown content to prevent headings from being misparsed as code blocks due to pasted indented content.
 * How it works: find the smallest common indentation among all non-empty lines and remove it from the start of each line (similar to Python textwrap.dedent).
 */
...
const CreateDocumentPage: React.FC = () => {
  ...
  // Read pre-filled content from URL params (navigated from the AI Writing page)
  // Note: searchParams.get() already decodes automatically, no need to call decodeURIComponent again
  const prefilledTitle = searchParams.get('title') || '';
  const prefilledContent = searchParams.get('content') || '';

  // Automatically extract a summary from the content (strip Markdown markup, take the first 200 characters)
  const generateSummary = (text: string): string => {
    if (!text) return '';
    const plain = text
      .replace(/^#{1,6}\s+/gm, '')     // Heading markers
      .replace(/\*\*(.+?)\*\*/g, '$1')  // Bold
      .replace(/[*_]{1,2}(.+?)[*_]{1,2}/g, '$1') // Italic
  ...
  // ========== Auto-save ==========
  // Build the form data object required for auto-save (recomputed on every render)
  ...
  const {
    ...
    documentId: undefined, // New document, no fixed ID yet
    formData: autoSaveFormData,
    onDocumentCreated: (docId: string) => {
      console.log('[AutoSave] New document created, ID:', docId);
    },
  });
  ...
  useEffect(() => {
    if (recoveryDraft) {
      // Only run after the user confirms the recovery
  ...
  const handleAcceptRecovery = () => {
    ...
    if (draft) {
      // Fill the draft data back into the form
  ...
  // Automatically fill teamId from the team context (store or URL params)
  useEffect(() => {
  ...
  const handleDirectPublish = async () => {
    ...
    try {
      ...
      let docId: string;
      if (currentDocId) {
        // Auto-save already created the document, update and publish it directly
  ...
  const handleSaveDraft = async () => {
    ...
    setLoading(true);
    try {
      // Convert the data format to match the backend API
      const documentData = {
        title,
        content,
        summary,
        categoryId: categoryId || undefined,
        teamId: teamId || undefined,
        tags: tags.join(','), // Convert to a comma-separated string
        status: 0, // 0-draft
      ...
      console.log('Saving document draft, request data:', documentData);
      if (currentDocId) {
        await documentService.updateDocument(currentDocId, documentData as any);
  ...
  const handleSubmitForReview = async () => {
    ...
    try {
      ...
      const documentData = {
        ...
        teamId: teamId || undefined,
        tags: tags.join(','), // Convert to a comma-separated string
        status: 0, // Save as draft first; submitForReview will change it to pending review
        documentType: 1, // 1-article
        allowComment: allowComments ? 1 : 0,
        source: 1, // 1-original
      };
      ...
      let docId: string;
      if (currentDocId) {
        // Auto-save already created the document, update it directly
        await documentService.updateDocument(currentDocId, documentData as any);
        docId = currentDocId;
      } else {
        // Not auto-saved yet, create a new document
            ...
            <div style={{
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  An intelligent writing assistant powered by Claude 3.5 Opus, helping you quickly create professional documents.
                </div>
                <div style={{
                  display: 'flex',
                  flexDirection: 'column',
