package com.knowledge.base.file.filter;

import com.knowledge.base.common.result.Result;
import com.knowledge.base.common.utils.UserContextUtil;
import com.knowledge.base.file.dto.TokenValidateVO;
import com.knowledge.base.file.feign.UserAuthFeignClient;
...
/**
 * JWT authentication filter (dedicated to the file service)
 *
 * <p>Delegates JWT token validation to the kb-user-auth microservice via Feign, rather
 * than trusting the gateway alone. Previously this service had no filter at all
 * (Spring Security config was anyRequest().permitAll(), with /files/** explicitly
 * called out as "fully open"), so uploads, downloads, and deletes were reachable
 * with zero authentication by anything that could reach the service directly -
 * FileController.uploadFile/uploadFiles read the uploader identity via
 * UserContextUtil.getUserIdFromHeader(request), which trusted a forged X-User-Id
 * header outright with no verification.</p>
 *
 * @author airwzz999
 * @since 1.0.0
 */
